...
OSBRICK_CONN_FILE="/tmp/apollo_fc_osbrick_connection.json"
OSBRICK_CONNECT_LOG="/tmp/apollo_fc_osbrick_connect.log"
...
conn_file = "/tmp/apollo_fc_osbrick_connection.json"
...
with open("/tmp/apollo_fc_osbrick_connection.json", "w", encoding="utf-8") as handle:
...
import json
with open('/tmp/apollo_fc_osbrick_connection.json', 'r', encoding='utf-8') as handle:
    payload = json.load(handle)
...
echo "[$(ts)] [PASS] Apollo FC LXD os-brick consumer test passed"
